...
# OAuth local http
if not (RAILWAY_ENVIRONMENT or FLY_ENVIRONMENT):
...
def ensure_database_ready():
    ...
    cursor.execute(
        "UPDATE administradores SET senha_hash = ? WHERE lower(email) = ?",
...
def catalogo():
    ...
    for cat in categorias_db:
        ...
        categorias.append({"nome": cat["categoria"], "itens": itens})
...
def login():
    ...
    return render_template("login.html", discord_url=discord_url, usuario_nome=get_current_user_name())
...
def callback():
    ...
    usuario_db = db.execute(
        "SELECT * FROM usuarios WHERE discord_id = ? OR lower(email) = ?",
        (discord_id, email.lower())
    ).fetchone()
...
def enviar_ticket():
    ...
    flash(f"Ticket #{ticket_id} criado com sucesso.", "success")
    return redirect(url_for("painel"))
...
def admin_tickets():
    ...
    return render_template("admin_tickets.html", tickets=tickets_admin, admin_nome=session.get("usuario_nome"))
...
def admin_adicionar_admin():
    ...
    db = get_db()
    existe = db.execute(
        "SELECT id FROM administradores WHERE lower(email) = ?",
        (email,)
    ).fetchone()
